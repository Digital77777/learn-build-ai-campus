-- =====================================================
-- Community Feature Production Readiness Migration (Fixed)
-- =====================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'insight_likes_user_insight_unique'
  ) THEN
    ALTER TABLE insight_likes
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'event_attendees_user_event_unique'
  ) THEN
    ALTER TABLE event_attendees
...
-- 6. Ensure reply count triggers exist
